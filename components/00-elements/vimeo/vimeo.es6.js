class LiteVimeo extends (globalThis.HTMLElement ?? class {}) {
  ...
  connectedCallback() {
    ...
    i >= 2 && (i *= 0.75),
      ...
      (t = Math.round(t * i)),
      fetch(
        `https://vimeo.com/api/oembed.json?url=https://player.vimeo.com/video/${this.videoId}`,
      )
        .then((e) => e.json())
        .then((i) => {
          let o = i.thumbnail_url;
          (o = o.replace(/-d_[\dx]+/i, `-d_${e}x${t}`)),
    ...
    (this.playLabel =
      ...
      o ||
        ...
        o.setAttribute("aria-label", this.playLabel),
        o.classList.add("ltv-playbtn"),
        this.append(o)),
  ...
  addIframe(autoplay = false) {
    ...
    if (autoplay) {
      e.setAttribute(
        "src",
        e.getAttribute("src") + "?autoplay=1&muted=1&loop=1&autopause=false",
      );
    ...
    if ("Vimeo" in globalThis) {
      ...
      console.warn(
        "Vimeo Player API not loaded. Please ensure the Vimeo Player script is included.",
      );
    }
...
Drupal.behaviors.LiteVimeo = {
  attach: (context) => {
    ...
    const autoplayVideos = once(
      "lite-vimeo",
      `lite-vimeo[autoplay='true']`,
      context,
    );
