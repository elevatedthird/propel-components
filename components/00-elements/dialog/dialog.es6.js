      ...
      modals.forEach((element) => {
        ...
        if (behaviorName in Drupal.behaviors) {
          if ('options' in Drupal.behaviors[behaviorName]) {
            ...
            console.error("kineticModal: Expected Drupal.behaviors.%s.options, but it's undefined. Please add an options key containing Micromodal options");
          }
          if (typeof Drupal.behaviors[behaviorName].init === 'function') {
            Drupal.behaviors[behaviorName].init(mm, element);
          } else {
            console.error("kineticModal: Expected Drupal.behaviors.%s.init, but it's undefined. Please add an init function to your JS file.");
          }
        }
      });
