    ...
    sliders.forEach((element) => {
      ...
      if (behaviorName in Drupal.behaviors) {
        if ("options" in Drupal.behaviors[behaviorName]) {
          ...
          console.error(
            "kineticGlide: Expected Drupal.behaviors.%s.options, but it's undefined. Please add an options key containing Glide.js options",
          );
        }
        if (typeof Drupal.behaviors[behaviorName].init === "function") {
          ...
          console.error(
            "kineticGlide: Expected Drupal.behaviors.%s.init, but it's undefined. Please add an init function to your JS file.",
          );
        }
      } else {
        console.error(
          "kineticGlide: Expected Drupal.behaviors.%s, but it's undefined.",
          behaviorName,
        );
      }
    });
